...
public enum SearchOperation {
    GREATER_THAN {
        ...
        public Predicate buildPredicate(
                Root<?> root, SearchCriteria criteria, CriteriaBuilder builder) {
    ...
    JOIN_GRAND_CHILD {
        ...
        public Predicate buildPredicate(
                Root<?> root, SearchCriteria criteria, CriteriaBuilder builder) {
            Join<Object, Object> join = root.join(criteria.getKey()).join(criteria.getChildKey());
            ...
            if (Enum.class.isAssignableFrom(join.get(criteria.getGrandChildKey()).getJavaType())) {
                return builder.equal(
                        join.get(criteria.getGrandChildKey()),
                        castToRequiredType(
                                join.get(criteria.getGrandChildKey()).getJavaType(), criteria.getValue()));
            }
            return builder.like(
                    builder.lower(join.get(criteria.getGrandChildKey())),
    ...
    JOIN_CHILD {
        ...
        public Predicate buildPredicate(
                Root<?> root, SearchCriteria criteria, CriteriaBuilder builder) {
            Join<Object, Object> join = root.join(criteria.getKey());
            ...
            if (Enum.class.isAssignableFrom(join.get(criteria.getChildKey()).getJavaType())) {
                return builder.equal(
                        join.get(criteria.getChildKey()),
                        castToRequiredType(
                                join.get(criteria.getChildKey()).getJavaType(), criteria.getValue()));
            }
            return builder.like(
                    builder.lower(join.get(criteria.getChildKey())),
                    PERCENT_SIGN + criteria.getValue().toString().toLowerCase() + PERCENT_SIGN);
        }
    },
    MATCH {
